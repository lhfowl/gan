...
class UtilsTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_1class_batch_trace_sqrt_product_value(self):
    from tensorflow_gan.python.eval.classifier_metrics import batch_trace_sqrt_product
    """Test that `batch_trace_sqrt_product` gives the correct value."""
    np.random.seed(0)

    # Make num_examples > num_features to ensure scipy's sqrtm function
    # doesn't return a complex matrix.
    test_pool_real_a = np.float32(np.random.randn(1, 512, 256))
    test_pool_gen_a = np.float32(np.random.randn(1, 512, 256))

    cov_real = np.expand_dims(np.cov(test_pool_real_a[0], rowvar=False), 0)
    cov_gen = np.expand_dims(np.cov(test_pool_gen_a[0], rowvar=False), 0)

    trace_sqrt_prod_op = batch_trace_sqrt_product(cov_real, cov_gen)

    with self.cached_session() as sess:
      actual_tsp = sess.run(trace_sqrt_prod_op)

    expected_tsp = np.expand_dims(_expected_trace_sqrt_product(cov_real[0], cov_gen[0]), 0)

    self.assertAllClose(actual_tsp, expected_tsp, 0.01)
  
  def test_2class_batch_trace_sqrt_product_value(self):
    from tensorflow_gan.python.eval.classifier_metrics import batch_trace_sqrt_product
    """Test that `batch_trace_sqrt_product` gives the correct value."""
    np.random.seed(0)
    ...
    # Make num_examples > num_features to ensure scipy's sqrtm function
    # doesn't return a complex matrix.
    test_pool_real_a = np.float32(np.random.randn(2, 512, 256))
    test_pool_gen_a = np.float32(np.random.randn(2, 512, 256))

    cov_real0 = np.cov(test_pool_real_a[0], rowvar=False)
    cov_gen0 = np.cov(test_pool_gen_a[0], rowvar=False)
    cov_real1 = np.cov(test_pool_real_a[1], rowvar=False)
    cov_gen1 = np.cov(test_pool_gen_a[1], rowvar=False)
    
    cov_real = np.stack([cov_real0, cov_real1], axis=0)
    cov_gen = np.stack([cov_gen0, cov_gen1], axis=0)
    ...
    expected_tsp0 = _expected_trace_sqrt_product(cov_real0, cov_gen0)
    expected_tsp1 = _expected_trace_sqrt_product(cov_real1, cov_gen1)
    expected_tsp = np.stack([expected_tsp0, expected_tsp1], axis=0)
